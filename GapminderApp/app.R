...
output_variables <- select(gapminder, c(year, lifeExp, pop, gdpPercap))
...
ui <- fluidPage(
    ...
    sidebarLayout(
        sidebarPanel("You can input here: ",
                     ...
                     hr(),
                     varSelectInput("my_variable", "Select the data you want to see", data = gapminder, selected = "gdpPercap", multiple = FALSE),
                     hr(),
                     selectInput("my_select", "Select a continent you wish to see the overview of ", choices = unique(gapminder$continent)),
